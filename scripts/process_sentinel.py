...
def merge_tif_files(directory, zone):
    ...
    for date, file_paths in tqdm(files_by_date.items()):
        if len(file_paths) > 1:
            ...
            merged_filename = os.path.join(directory, f"{zone}_{date}_merged.tif")
...
parser.add_argument('--zone', type=str, required=True, help='Name of the zone')
...
for input_file in glob(input_folder + '/*SAFE.zip'):
    wkt_file_path = f'{shapefile_path[:-4]}.wkt'
...
merge_tif_files(input_folder, zone)