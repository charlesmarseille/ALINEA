...
def get_tiles(date, day_offset):
    start_date = (date - timedelta(days=day_offset)).strftime("%Y-%m-%d")
    end_date = (date + timedelta(days=day_offset)).strftime("%Y-%m-%d")

    print('Get tiles in interval: ', start_date, end_date)
    ...
    for idx, row in gdf.iterrows():
        ...
        if p.shape[0] > 0:
            ...
            return False    #no data was found for the given date in the specified interval (date - day_offset, date + day_offset). The script will try again with day_offset+1.
